<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.type" placeholder="Country" clearable class="filter-item" style="width: 130px">
...
<!--          <span>{{ row.time | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
      ...
      <el-table-column label="Cert" width="400px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.fingerprint }}</span>
        </template>
      </el-table-column>
      <el-table-column label="IP" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.ip }}</span>
<!--          <el-tag>{{ row.country | typeFilter }}</el-tag>-->
          <el-tag>{{ row.country }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column label="Port" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.port }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle } from '@/api/bridge'
...
export default {
  ...
  data() {
    return {
      ...
      importanceOptions: ['published', 'draft', 'deleted'],
      calendarTypeOptions,
